...
class MoveItMotionPlanner(Node):
    ...
    def add_collision_objects(self):
        ...
        object_positions = [
            (0.0, 0.9, 0.46526), # Table
            #(0.0, 0.0, 0.5)  # Position of the robot body
            # (0.0, 0.0, 1.1)   # Position of the upper box
            ]
        object_dimensions = [
            (1.8, 1.62, 0.1), # Table
            #(0.4, 0.2, 1.0)  # Dimensions of the robot body (width, depth, height)
            # (1.1, 0.2, 0.21)   # Dimensions of the upper box (width, depth, height)
            ]
...
class TkinterGUI:
    ...
    def create_buttons(self):
        ...
        def create_arm_hand_buttons(side, col):
            ...
            tk.Button(self.root, text="Extended State", command=lambda: self.plan_arm_to_predefined_state(side, "extended")).grid(row=2, column=col, pady=5, padx=5, sticky='ew')
        ...
        tk.Button(self.root, text="Extended Simultaneously", command=lambda: self.executesimultaneously("extended")).grid(row=2, column=2, pady=5, padx=5, sticky='ew')
    ...
    def standbymode(self):
        self.motion_planner.plan_both_arms_to_predefined_state("humanoid4.1")
        time.sleep(1.0)
        self.motion_planner.plan_both_arms_to_predefined_state("humanoid4.2")
        time.sleep(1.0)
        self.motion_planner.plan_both_arms_to_predefined_state("humanoid4.3")
        time.sleep(1.0)
        self.motion_planner.plan_both_arms_to_predefined_state("humanoid4")
        messagebox.showinfo("Info", f"Planning both arms standy mode completed.")
    ...
    def magazine_loading(self):
        ...
        # 0.0707 = 0.1cos45;  0.0778=0.11cos45
        ...
        time.sleep(1.0)
        self.motion_planner.plan_both_arms_to_target(orientDownwards,orientDownwards,"tf_magazine", "tf_bullet1",(0,-0.02,0.2),(0,0,0.2))
        self.motion_planner.plan_arm_to_target('right',orientLeftwards,"tf_sphere",(-0.11,0,0.11))
        ...
        for bullet in bullets:
            ...
            self.motion_planner.openrightgripper.publish(msgrightopen)
            time.sleep(0.5)
            self.motion_planner.plan_arm_to_target('right', orientLeftwards, "tf_sphere", (-0.3,0,0.11))          
        ...
        left_end_goal.pose.position.z = 0.55+0.12
        ...
        self.motion_planner.openleftgripper.publish(msgleftopen)
        time.sleep(0.5)
        left_end_goal.pose.position.z = 0.55+0.2
        self.motion_planner.plan_arm_with_pose_stamped('left',left_end_goal)
        self.motion_planner.plan_both_arms_to_predefined_state('ready')
